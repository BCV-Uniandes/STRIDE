...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    for samples, targets in metric_logger.log_every(data_loader, print_freq, header, logger=logger):
        ...
        if 'regression_loss' in loss_dict_reduced:
            metric_logger.update(regression_loss=loss_dict_reduced['regression_loss'])
...
def evaluate(model, criterion, postprocessors, data_loader, base_ds, device, output_dir, wo_class_error=False, args=None, logger=None):
    ...
    if args.regression:
        iou_types = tuple(list(iou_types)+['regression'])
    useCats = True
    try:
    ...
    if coco_evaluator is not None:
        ...
        if args.save_results:
            coco_evaluator.save_coco_json(output_dir)
    ...
    if coco_evaluator is not None:
        ...
        if args.regression:
            stats['coco_eval_regression'] = coco_evaluator.coco_eval['regression'].stats
