...
class BackboneBase(nn.Module):
    ...
    def __init__(self, backbone: nn.Module, train_backbone: bool, num_channels: int, return_interm_indices: list, regression: bool, out_feats: bool):
        super().__init__()
        ...
        # breakpoint()
        if regression or out_feats:
            self.reg_conv = nn.Sequential(nn.Conv2d(2048,256,(1,1)),
                                          nn.AdaptiveAvgPool2d((1,1)))
            if regression:
                self.regressor = nn.Sequential(nn.Linear(256,27),
                                            nn.Sigmoid())
    ...
    def forward(self, tensor_list: NestedTensor):
        ...
        if self.regression and self.out_feats:
            trans = self.reg_conv(xs['3'])
            trans = torch.flatten(trans, 1)
            regres = self.regressor(trans)
            return out, trans, regres
        
        if self.regression:
            trans = self.reg_conv(xs['3'])
            trans = torch.flatten(trans, 1)
            regres = self.regressor(trans)
            return out, None, regres
...
class Backbone(BackboneBase):
    ...
    def __init__(self, name: str,
        ...
        super().__init__(backbone, train_backbone, num_channels, return_interm_indices, regression, out_feats)
...
class Joiner(nn.Sequential):
    ...
    def forward(self, tensor_list: NestedTensor):
        xs, out_feats, reg = self[0](tensor_list)
...
def build_backbone(args):
    ...
    if args.backbone in ['resnet50', 'resnet101']:
        backbone = Backbone(args.backbone, train_backbone, args.dilation,   
                                ...
                                batch_norm=FrozenBatchNorm2d,
                                regression=args.regression and args.backbone_reg,
                                out_feats=args.regression and args.backbone_out,)
